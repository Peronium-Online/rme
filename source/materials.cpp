...
bool Materials::loadExtensions(FileName directoryName, wxString &error, wxArrayString &warnings)
{
	...
	do
	{
		std::cout << "Loading extension at: " << filename << std::endl;
		FileName fn;
		...
		MaterialsExtension *materialExtension = new MaterialsExtension(extensionName, extensionAuthor, extensionDescription);
...
bool Materials::isInTileset(Item *item, std::string tilesetName) const
{
	...
	return type.id != 0 && (isInTileset(type.brush, tilesetName) ||
